...
class CtaTemplate(object):
    ...
    #----------------------------------------------------------------------
    def chulikaipingcang(self, celve, tick, bar=None):
        selfpos = 0
        price = None
        if tick is None and bar is not None:
            price = bar.close
        if tick is not None:
            price = tick.lastPrice
        if self.weituopos == 0 and celve == 100:
            if self.pos == 0:
                self.weituopos = 1
                self.buy(price, 1)
                # 如果有空头持仓，则先平空，再做多
        ...
        elif self.weituopos == 1 and celve == 50:
            if self.pos > selfpos:
                self.weituopos = 0
                self.sell(price, 1)
        elif self.weituopos == 0 and celve == 200:
            if self.pos == selfpos:
                self.weituopos = -1
                self.short(price, 1)
            elif self.pos > selfpos:
                ...
                self.short(price, 1)
                self.weituopos = -1
        elif self.weituopos == -1 and celve == -250:
            if self.pos < selfpos:
                self.weituopos += 1
                self.cover(price, 1)
    def onStart(self):
...
class ArrayManager(object):
    ...
    def endBar(self):
        ...
        return zhibiao(self.diff,self.dea,self.macd)
        
    #----------------------------------------------------------------------
    def updateTick(self,tick):
        self.macd = self.caculateMACD(tick.lastPrice)
        return zhibiao(self.diff,self.dea,self.macd)
    #----------------------------------------------------------------------
