...
def downMinuteBarBySymbol(symbol):
    ...
    df = ts.bar(symbol,cons,start_date='2018-01-10',end_date='2018-04-09',freq='1min',asset='X')
    df = df.sort_index()
...
def downtickBySymbol(symbol):
    ...
    df = df.sort_index()
    for ix,row in df.iterrows():
    #    tick = generateVtTick(row)
    #     for name,de in row.items():
    #         print name,',',de
        for key in row:
            print key
        return
