...
class ZeroStrategy(CtaTemplate):
    ...
    def onStop(self):
        ...
        self.putEvent()
    def cancelVtOrder(self,order,yuanyin):
        print(yuanyin)
    ...
    def onTick(self, tick):
        ...
        if not self.stopcount:
            ...
            if tick.datetime.second > 50:
                ...
                if am.macd > 0 and am.lastmacd < 0:
                    if self.pos>0:

                       fangxiang = duoping
                       price = tick.askPrice1
                    elif self.longOrder is not None:
                        self.cancelVtOrder(self.longOrder,u'平多仓时候')

                if am.diff < 0 and am.macd > 0 and am.lastmacd < 0:

                    self.chulikaipingcang(kong,tick.askPrice1)
                    fangxiang = kong
                    price = tick.askPrice1
                ...
                if am.macd < 0 and am.lastmacd > 0:
                    if self.pos < 0:
                      ...
                      price = tick.bidPrice1
                        ...
                        self.cancelVtOrder(self.shortOrder,u'平空仓时候')
    ...
    def reactOrder(self,order,vtOrderID):
        if (order.direction == DIRECTION_LONG and order.offset == OFFSET_OPEN) \
                or \
                (order.direction == DIRECTION_SHORT and order.offset == OFFSET_CLOSE):
