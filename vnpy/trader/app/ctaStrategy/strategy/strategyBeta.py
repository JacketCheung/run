...
class tempStrategy(CtaTemplate):
    ...
    def barcelve(self, zhibiao, price):
        for celve in self.barCelvezu:
            xinhao = celve.celveOnbar(zhibiao, self.lastzhibiao,price)
            if xinhao == 100:
    ...
    def onBar(self, bar):
        ...
        jisuan = zhibiao(diff,dea,macd,bar.close)
        if not self.stopcount:
            self.barcelve(jisuan)
...
class lianmiancelve(celvemoban):
    ...
    def celveOnbar(self, zhibiao, lastzhibiao):
        '''bar策略'''
        self.duocelveOnbar(zhibiao,lastzhibiao)
        self.kongcelveOnbar(zhibiao,lastzhibiao)
        raise NotImplemented
    ...
    def duocelveOnbar(self, zhibiao):
        pass

    def kongcelveOnbar(self, zhibiao,lastzhibiao,price):
        if self.lowestMACD is None or self.lowestPrice is None:
            if zhibiao.macd - lastzhibiao.macd > 0 and lastzhibiao.macd < 0:
                self.lowestMACD = lastzhibiao.macd
                self.lowestPrice = lastzhibiao.price
