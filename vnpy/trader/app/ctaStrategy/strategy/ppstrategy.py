...
class DoubleMaStrategy(CtaTemplate):
    ...
    #----------------------------------------------------------------------
    def tickcelve(self,zhibiao,price):
        for celve in self.tickCelvezu:
          self.chulikaipingcang(celve.celveOntick(zhibiao,self.lastzhibiao),price)
    def barcelve(self,zhibiao,price):
        for celve in self.barCelvezu:
            xinhao = celve.celveOnbar(zhibiao,self.lastzhibiao)
            if xinhao == duo:
                print(zhibiao.macd,self.lastzhibiao.macd,self.lastzhibiao.dea,self.lastzhibiao.dea > - self.celve0.canshu,self.lastzhibiao.dea < self.celve0.canshu,self.celve0.canshu,price,'kaicangduo')
    ...
    def onTick(self, tick):
        ...
        self.tickcelve(zhibiao,tick.lastPrice)
    #----------------------------------------------------------------------
    def onBar(self, bar):
        """收到Bar推送（必须由用户继承实现）"""
        am = self.am        
...
class ceshi(celvemoban):
    def celveOnbar(self,zhibiao,lastzhibiao):
        caozuo = 0
        if  zhibiao.macd > 0 and lastzhibiao.macd < 0 :
            caozuo = self.cangweishibie(duo)
        # else:print('zanting', lastzhibiao is None,lastzhibiao.diff,lastzhibiao.dea)
        if zhibiao.macd < 0 and lastzhibiao.macd > 0:
...
class zerocelve(celvemoban):
    ...
    def celveOnbar(self, zhibiao,lastzhibiao):
        self.fuhetiaojian(lastzhibiao)
        return self.celveOntick(zhibiao,lastzhibiao)
    ...
    def fuhetiaojian(self, zhibiao):
        print('always',zhibiao.dea)
        if zhibiao is not None and zhibiao.dea < self.canshu and zhibiao.dea > -self.canshu:
            print('tiaojian',zhibiao.dea < self.canshu and zhibiao.dea > -self.canshu,zhibiao.dea,self.canshu)
            self.tiaojian = True
        else:
            self.tiaojian =  False
...
class zhibiao():
    ...
    def dj(self,lastzhibiao):
        return self.diff - lastzhibiao.diff > 0